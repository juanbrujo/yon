...
module.exports = (robot) ->
  ...
  robot.hear /@?(\S*)(\b(?:\+\+|--))(\s|$)/, (response) ->
    ...
    return response.send "Oe el karma es pa otros no pa ti!" if thisUser is targetUser
    op = response.match[2]
  ...
  robot.hear /^karma(?:\s+@?(.*))?$/, (response) ->
    ...
    if targetToken.toLowerCase() in ["todos", "all"]
      ...
      thisUser = response.message.user
      if thisUser.name.toLowerCase() != "jorgeepunan"
        response.send "Tienes que ser _jorgeepunan_ para realizar esta función"
        return
